...
GLuint drawChar_To_Texture(const char* s) {
	...
	{
		int width, x, y;
		GetCharWidth32W(hDC, w, w, &width);    // È¡µÃ×Ö·ûµÄ¿í¶È
		x = (TEXTURE_SIZE - width) / 2;
		y = FONT_SIZE / 8;
		//glWindowPos2iARB(x, y); // Ò»¸öÀ©Õ¹º¯Êý
	}

	// »æÖÆ×Ö·û
	// »æÖÆÇ°Ó¦¸Ã½«¸÷ÖÖ¿ÉÄÜÓ°Ïì×Ö·ûÑÕÉ«µÄÐ§¹û¹Ø±Õ
	// ÒÔ±£Ö¤ÄÜ¹»»æÖÆ³ö°×É«µÄ×Ö·û
	{
	...
	// ¸´ÖÆ×Ö·ûÏñËØµ½ÎÆÀí
	// ×¢ÒâÎÆÀíµÄ¸ñÊ½
	// ²»Ê¹ÓÃÍ¨³£µÄGL_RGBA£¬¶øÊ¹ÓÃGL_LUMINANCE4
	// ÒòÎª×Ö·û±¾À´Ö»ÓÐÒ»ÖÖÑÕÉ«£¬Ê¹ÓÃGL_RGBAÀË·ÑÁË´æ´¢¿Õ¼ä
	// GL_RGBA¿ÉÄÜÕ¼16Î»»òÕß32Î»£¬¶øGL_LUMINANCE4Ö»Õ¼4Î»
	{
		GLuint texID;
...
void drawString(const char* str) {
	...
	if (isFirstCall) { // Èç¹ûÊÇµÚÒ»´Îµ÷ÓÃ£¬Ö´ÐÐ³õÊ¼»¯
		// ÎªÃ¿Ò»¸öASCII×Ö·û²úÉúÒ»¸öÏÔÊ¾ÁÐ±í
		isFirstCall = 0;

		// ÉêÇëMAX_CHAR¸öÁ¬ÐøµÄÏÔÊ¾ÁÐ±í±àºÅ
		lists = glGenLists(128);

		// °ÑÃ¿¸ö×Ö·ûµÄ»æÖÆÃüÁî¶¼×°µ½¶ÔÓ¦µÄÏÔÊ¾ÁÐ±íÖÐ
		wglUseFontBitmaps(wglGetCurrentDC(), 0, 128, lists);
	}
	// µ÷ÓÃÃ¿¸ö×Ö·û¶ÔÓ¦µÄÏÔÊ¾ÁÐ±í£¬»æÖÆÃ¿¸ö×Ö·û
	for (; *str != '\0'; ++str)
		glCallList(lists + *str);
}
HWND CreateEdit(
	const HWND hParent,      //¸¸´°¿ÚÀà
	const HINSTANCE hInst,   //Ó¦ÓÃ³ÌÐòÊµÀý
	DWORD dwStyle,                //´°¿ÚÑùÊ½
	const RECT rc,                  //Ïà¶ÔÓÚ¸¸´°¿ÚµÄÎ»ÖÃ¾ØÐÎ
	const int id,                  //ÎÄ±¾¿òIDºÅ
	const char* caption)  //ÎÄ±¾¿ò±êÌâ£¨³õÊ¼ÎÄ×Ö£©
{
