...
int  IsCellOnLayer(SuperCubes*cube,Cells*cell,int idem,int ilayer){
	if (ilayer == 0)
		return 0;
	if (ilayer < 0)
		ilayer = cube->nseg + ilayer + 1;
	double dw = cube->width / cube->nseg;
	if (ilayer == 1)
	{
		if (fabs(cell->loc[idem] + dw*cube->nseg / 2.) < dw)
			return 1;
		else
			return 0;
	}
	...
	if (fabs(cell->loc[idem] - dw*(ilayer - cube->nseg / 2. - 0.5)) < dw / 4)
		return 1;
	else
		return 0;
	return 0;
...
void UpdateCellFace(SuperCubes*cube, Cells* cell){
	...
	for (int idem = 0; idem < NDEM; idem++)
	{
		if (fabs(cell->loc[idem] + dw*cube->nseg / 2.) < dw / 2)
		{
			cell->iface = idem;
		}
		if (fabs(cell->loc[idem] - dw*cube->nseg / 2.) < dw / 2)
...
void FillCell(SuperCubes* cube)
{
	...
	for (int idx1 = 0; idx1 < cube->nseg; idx1++)
	for (int idx2 = 0; idx2 < cube->nseg; idx2++)
	{
		if (iface >= NDEM)
			cube->cell[icell].loc[(iface%NDEM) % NDEM] = cube->center[(iface%NDEM) % NDEM] + dw*cube->nseg / 2.;
		else
			cube->cell[icell].loc[(iface%NDEM) % NDEM] = cube->center[(iface%NDEM) % NDEM] - dw*cube->nseg / 2.;
		cube->cell[icell].loc[(iface%NDEM + 1) % NDEM] = cube->center[(iface%NDEM + 1) % NDEM] + dw*(idx1 - cube->nseg / 2. + 0.5);
		cube->cell[icell].loc[(iface%NDEM + NDEM - 1) % NDEM] = cube->center[(iface%NDEM + NDEM - 1) % NDEM] + dw*(idx2 - cube->nseg / 2. + 0.5);
...
void ShowSuperCube(SuperCubes* cube){
	qsort(cube->cell, cube->nseg*cube->nseg*NDEM * 2, sizeof(Cells), CompareCellLoc);
/*	for (int icell = 0; icell < NSEG*NSEG*NDEM * 2; icell++)
	...
	for (int iface = 0; iface < NDEM * 2; iface++)
	{
		...
		for (int icell = 0; icell < cube->nseg*cube->nseg*NDEM * 2; icell++)
	...
	glRasterPos3f(.6f, 0.f, 0.f);
	drawString("L");
	glColor3f(1.f, 0.f, 0.f);
	glRasterPos3f(-.6f, 0.f, 0.f);
	drawString("R");
	glColor3f(0.f, 1.f, 0.f);
	glRasterPos3f(0.f, .6f, .0f);
	drawString("F");
	glColor3f(0.f, 1.f, 0.f);
	glRasterPos3f(0.f, -.6f, .0f);
	drawString("B");
	glColor3f(0.f, 0.f, 1.f);
	glRasterPos3f(0.f, 0.f, .6f);
	drawString("U");
	glColor3f(0.f, 0.f, 1.f);
	glRasterPos3f(0.f, 0.f, -.6f);
	drawString("D");
